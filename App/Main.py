...
from App.Const import RIGHT_COLOR, WRONG_COLOR, BG_COLOR, TEXT_COLOR
from App.Const import MENU_BG_COLOR, MENU_HEIGHT, MENU_TEXT_COLOR, MENU_WIDTH
from App.Const import BUTTON_HEIGHT, BUTTON_WIDTH, WIDTH, HEIGHT
from App.Const import CELL_SIZE, SPACE_SIZE
...
def drawWinMenu(window):
    pg.draw.rect(window, MENU_BG_COLOR,
                 (CELL_SIZE // 2, CELL_SIZE, MENU_WIDTH, MENU_HEIGHT))
    font1 = pg.font.Font('Fonts/UbuntuMono-Regular.ttf', 52)
    font2 = pg.font.Font('Fonts/UbuntuMono-Regular.ttf', 48)

    window.blit(font1.render("Вы выиграли!", True, MENU_TEXT_COLOR),
                (CELL_SIZE // 2 + 25, CELL_SIZE + 20))
    ...
    pg.draw.rect(window, color,
                 (CELL_SIZE // 2 + SPACE_SIZE,
                  CELL_SIZE + MENU_HEIGHT - SPACE_SIZE - BUTTON_HEIGHT,
    ...
    pg.draw.rect(window, color,
                 (CELL_SIZE // 2 + SPACE_SIZE * 2 + BUTTON_WIDTH,
                  CELL_SIZE + MENU_HEIGHT - SPACE_SIZE - BUTTON_HEIGHT,
                  BUTTON_WIDTH, BUTTON_HEIGHT))

    window.blit(font2.render("Заново", True, TEXT_COLOR),
                (CELL_SIZE // 2 + 15, CELL_SIZE + 120))
    window.blit(font2.render("Выход", True, TEXT_COLOR),
                (CELL_SIZE // 2 + 30 + SPACE_SIZE + BUTTON_WIDTH,
                 CELL_SIZE + 120))
...
def update():
    ...
    if not mouseClick:
        ...
        if win:
            ...
            if (x <= mx <= x + BUTTON_WIDTH) and \
               (y <= my <= y + BUTTON_HEIGHT):
                button1_move = True
